...
impl SignatureVerifier for AwsLcSignatureVerifier {
    fn verify_signature(
        ...
        match sig_alg {
            SignatureAlgorithm::ECDSA => {
                self.verify_ecdsa(cert_der, data, signature, hash_alg)
            }
            SignatureAlgorithm::RSA => {
                self.verify_rsa(cert_der, data, signature, hash_alg)
            }
            _ => Err(format!("Unsupported signature algorithm: {:?}", sig_alg)),
        }
    }
}

impl AwsLcSignatureVerifier {
    fn verify_ecdsa(
        &self,
        cert_der: &[u8],
        data: &[u8],
        ...
        check_verify_scheme(SignatureAlgorithm::ECDSA, hash_alg, group)?;
    ...
    fn verify_rsa(
        ...
        let algorithm: &dyn aws_lc_rs::signature::VerificationAlgorithm = match hash_alg {
            HashAlgorithm::SHA256 => &RSA_PKCS1_2048_8192_SHA256,
            HashAlgorithm::SHA384 => &RSA_PKCS1_2048_8192_SHA384,
            _ => return Err(format!("RSA: unsupported hash algorithm {:?}", hash_alg)),
        };

        let public_key = UnparsedPublicKey::new(algorithm, pubkey_bytes);
        public_key.verify(data, signature).map_err(|_| {
            format!("RSA signature verification failed for {:?}", hash_alg)
        })
    }
}
