...
impl State {
    ...
    fn await_server_hello(self, client: &mut Client) -> Result<Self, Error> {
        ...
        // Enforce cipher suite is supported and allowed by config.
        // We accept any certificate-based suite (ECDSA or RSA) because the
        // suite's signature algorithm refers to the server's authentication.
        let is_compatible = if cs.is_psk() {
            client
                .engine
                .crypto_context()
...
fn handshake_create_client_hello(
    ...
    // Get cipher suites from config. We offer all certificate-based suites
    // regardless of our own key type — the suite's signature algorithm refers
    // to the SERVER's authentication, not the client's. PSK suites are only
    // offered when a PSK resolver is configured.
    let cipher_suites: CipherSuiteVec = engine
        ...
        .filter(|suite| {
            if suite.is_psk() {
                engine.crypto_context().is_cipher_suite_compatible(*suite)
            } else {
                true
            }
        })
